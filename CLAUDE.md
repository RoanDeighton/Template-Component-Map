...
2. **App UI copy**: hardcoded strings in `web/app/**` and `web/components/**` that render as visible text, headings, captions, empty states, labels. These are client-facing even though they live in code — easy to forget since they sit next to className strings and JSX, but a site visitor reads them same as any markdown copy. Watch for internal/dev jargon slipping in from the surrounding code (e.g. "chrome" meaning UI framing, caught and fixed in this file's own history) — write what a visitor would recognize, not implementation vocabulary.
